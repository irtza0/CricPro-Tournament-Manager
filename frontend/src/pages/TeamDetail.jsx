        ...
        <div className="flex items-center gap-5">
          <div className="w-20 h-20 rounded-2xl overflow-hidden bg-surface-800/60 border border-surface-700/30 flex items-center justify-center shadow-lg">
            {TEAM_LOGOS[team.name] ?
              <img src={TEAM_LOGOS[team.name]} alt={team.name} className="w-full h-full object-cover" /> :
              <span className="text-4xl font-bold text-primary-400 font-display">{team.short_name?.[0]}</span>}
          </div>
          <div>
                  ...
                  <tr key={p.id} className="table-row">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="player-avatar w-8 h-8 text-[10px] font-bold text-white">{p.first_name[0]}{p.last_name[0]}</div>
